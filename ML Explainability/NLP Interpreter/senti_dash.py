...
model = tf.keras.models.load_model('models/senti_model_sigmoid_en.keras')

with open('models/tokenizer_senti_model_en.json') as fp:
    data = json.load(fp)
...
def run_senti_model(n_clicks, n_submit, user_input, sentiment_analysis):
    ...
    if user_input is None or user_input.isspace() is True:
        ...
        texto = user_input.translate(str.maketrans('', '', string.punctuation))
        texto = texto.lower()
        ...
        padded_text = tf.keras.preprocessing.sequence.pad_sequences(
            sequence, maxlen=250, truncating='post')
