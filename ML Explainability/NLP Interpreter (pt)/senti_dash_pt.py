...
model = keras.models.load_model('models\senti_model_sigmoid_portuguese.h5')


with open('models\\tokenizer_senti_model_portuguese.json') as fp:
    data = json.load(fp)
...
controls = dbc.InputGroup(
    ...
    children=[
        dbc.Input(id='user-input',
                  placeholder='Escreva um comentário...', type='text'),
...
def run_senti_model(n_clicks, n_submit, user_input, sentiment_analysis):
    ...
    if user_input is None or user_input == '':
        ...
        neg = f'Sentimento Negativo 😔 {pred_neg}'

        pred_pos = prediction[0][0]
        pred_pos = '{:,.2f}'.format(pred_pos * 100) + ' %'
        pos = f'Sentimento Positivo 😊 {pred_pos}'
